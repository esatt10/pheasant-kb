...
| POST | `/sources` | Register a runtime source (full schema). Accepts plugin types. A source whose connector never reads `path` — a plugin, `web_collection`, `api`, `s3` — takes `"/unused"` (or an empty path) instead of a real directory. A `web_collection` takes its pages in `urls` and gets `connector.allow_experimental: true` unless the request sets it. Same `sync_now`/`wait` fields as quick-add. |
